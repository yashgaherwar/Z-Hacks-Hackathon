...
#         # cv2.imshow('Communication Buddy', img)
#         key = cv2.waitKey(1)
...
def gen_frames(): 
    ...
    while True:
        ...
        #         label = np.argmax(result,axis=1)[0]
                ...
                # max_face = max(face_output,key=face_output.count)
                # max_speech = max(speech_ouput,key=speech_ouput.count)
                # f.write(max_face)
